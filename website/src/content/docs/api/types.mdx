...
These are also scene-level defaults: `textureBackend`, `textureProjection`,
`textureImageRendering`, and `textureLeafSizing` on `PolySceneOptions` (and the
matching `<PolyMesh>` props) set the default for every polygon, and a polygon's
own `texturePresentation` overrides it.
