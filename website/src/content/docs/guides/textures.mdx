...
The simplest way to render a mesh is the mesh element with a `src`. It fetches the file, parses it, and mounts one internal render leaf per visible polygon inside a `.polycss-mesh` wrapper automatically.
