...
import MorphSphereDemo from "../../../components/MorphSphereDemo.astro";
...
Animated Morph Sphere is a CC0, 960-polygon model that combines a morph clip
with a shape-transform clip while keeping the same DOM leaves mounted. Its
fixed, no-input loop is prepared into CSS transform keyframes at authoring
time, so the browser plays it without a JavaScript render loop.

<MorphSphereDemo />

The page validates and mounts the prepared model once. A generated playback
stylesheet owns the animation, and teardown only destroys the retained mount:
...
Use the caller-driven animation and deformation runtimes when weights respond
to input or application state. Fixed presentation loops can be prepared into
browser-driven CSS as this example is.
