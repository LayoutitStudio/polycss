...
Every parser normalizes coordinates (fit-to-target scale, Y-up → Z-up), and the
axis permutation is chosen specifically so it does *not* flip handedness.
...
file is deduped and optimized *before* `merge` is ever consulted. To render file
geometry exactly as authored, parse it with `parseObj` / `parseStl` /
