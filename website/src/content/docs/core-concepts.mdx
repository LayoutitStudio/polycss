...
| `<q>` | Cast shadow | Dynamic-lighting shadow leaves for meshes with `castShadow`. |

You normally do not target these tags directly; use `Poly`, `PolyMesh`, classes, data attributes, or render stats.
