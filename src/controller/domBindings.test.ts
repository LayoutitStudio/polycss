...
describe("mountCameraBinding", () => {
  ...
  describe("startAutoRotate() and stopAutoRotate()", () => {
    it("startAutoRotate updates animate prop", () => {
      const onSnapshot = vi.fn();
      const binding = mountCameraBinding(element, {}, onSnapshot);

      // Starting auto-rotate should not throw
      expect(() => binding.startAutoRotate(true)).not.toThrow();

      binding.destroy();
    });

    it("stopAutoRotate updates animate to false", () => {
      const onSnapshot = vi.fn();
      const binding = mountCameraBinding(element, { animate: true }, onSnapshot);

      expect(() => binding.stopAutoRotate()).not.toThrow();

      binding.destroy();
    });

    it("startAutoRotate with numeric speed", () => {
      const onSnapshot = vi.fn();
      const binding = mountCameraBinding(element, {}, onSnapshot);

      expect(() => binding.startAutoRotate(0.5)).not.toThrow();

      binding.destroy();
    });

    it("startAutoRotate with config object", () => {
      const onSnapshot = vi.fn();
      const binding = mountCameraBinding(element, {}, onSnapshot);

      expect(() => binding.startAutoRotate({ axis: "x", speed: 1, pauseOnInteraction: false })).not.toThrow();
