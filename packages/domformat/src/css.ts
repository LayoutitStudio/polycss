...
import type { DomStylesheetBinding } from "./public-types.js";
...
const SAFE_FUNCTIONS = new Set([
  ...
  "blur", "brightness",
  "calc", "circle", "clamp", "color", "color-mix", "conic-gradient", "contrast", "cos", "cubic-bezier",
  "drop-shadow",
  "ellipse", "exp",
  "fit-content",
  "grayscale",
...
const SAFE_PROPERTIES = new Set([
  ...
  "box-sizing",
  "color", "contain", "corner-bottom-left-shape", "corner-bottom-right-shape", "corner-top-left-shape", "corner-top-right-shape", "cursor",
  "display",
...
interface CssUrlSpan extends SourceRange {
...
interface CssAnalysis {
  ...
  readonly scopeSpans: readonly SourceRange[];
...
function trimRange(css: string, start: number, end: number): SourceRange {
  while (start < end && isWhitespace(css[start])) start += 1;
  while (end > start && isWhitespace(css[end - 1])) end -= 1;
  return { start, end };
}

function assertAlphaCharacters(css: string): void {
...
function splitTopLevel(css: string, start: number, end: number, delimiter: string, code: string, label: string): SourceRange[] {
...
function topLevelColon(css: string, start: number, end: number): number {
  let quote = "";
  let round = 0;
  let square = 0;
  for (let index = start; index < end; index += 1) {
...
function parseRules(css: string, limits: DomLimits) {
  const rules: Array<{ preludeStart: number; preludeEnd: number; bodyStart: number; bodyEnd: number }> = [];
  let index = 0;
...
function assertScopedSelector(selector: string, scope: string): void {
...
function matchingParen(css: string, open: number, end: number): number {
  let quote = "";
  let depth = 1;
...
function urlToken(css: string, open: number, close: number): string {
...
interface CssScanState {
  ...
  readonly urls: CssUrlSpan[];
  readonly scopeSpans: SourceRange[];
  selectors: number;
  declarations: number;
}

function scanFunctions(css: string, start: number, end: number, state: CssScanState): void {
  let quote = "";
  for (let index = start; index < end; index += 1) {
    const character = css[index];
    if (quote) {
    ...
    let open = cursor;
    while (open < end && isWhitespace(css[open])) open += 1;
    if (css[open] !== "(") {
      index = cursor - 1;
      continue;
    }
    invariant(open === cursor, "UNSAFE_CSS", "Stylesheet function names must immediately precede '('.");
    ...
    if (name === "url") {
      const close = matchingParen(css, open, end);
      const token = urlToken(css, open, close);
...
export function cssScopeAttribute(scope: unknown): Readonly<{ name: `data-${string}`; value: string }> {
...
function analyzeCss(css: string, binding: DomStylesheetBinding, limits: DomLimits, policy?: CssValidationPolicy): CssAnalysis {
  ...
  const state: CssScanState = { functions: 0, limits, urls: [], scopeSpans: [], selectors: 0, declarations: 0 };
...
export function rewriteCss(
  ...
  const replacements: Array<{ start: number; end: number; value: string }> = analysis.scopeSpans.map((span) => ({ ...span, value: scope }));
  for (const url of analysis.urls) {
