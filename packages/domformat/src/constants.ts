...
export interface DomJsonStructureLimits {
  readonly maxArrayItems: number;
  readonly maxObjectMembers: number;
  readonly maxKeyCodeUnits: number;
}

function cappedProduct(value: number, multiplier: number): number {
  return value > Math.floor(Number.MAX_SAFE_INTEGER / multiplier)
    ? Number.MAX_SAFE_INTEGER
    : value * multiplier;
}

export function jsonStructureLimits(limits: DomLimits = DEFAULT_LIMITS): DomJsonStructureLimits {
...
export function validateLimitOverrides(overrides: unknown = {}): DomLimitOverrides {
  ...
  for (const [name, value] of Object.entries(overrides)) {
    ...
    invariant(typeof value === "number" && Number.isSafeInteger(value) && value >= 0, "INVALID_LIMITS", `Limit ${name} must be a nonnegative safe integer.`);
  }
  return overrides as DomLimitOverrides;
}

export function mergeLimits(overrides: unknown = {}): DomLimits {
  const validated = validateLimitOverrides(overrides);
  return Object.freeze({ ...DEFAULT_LIMITS, ...validated });
}
