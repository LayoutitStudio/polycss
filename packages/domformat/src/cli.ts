...
import { DomFormatError, errorCode, invariant } from "./errors.js";
import { writeDomFiles } from "./package-files.js";
import { assertRealDirectory } from "./path-security.js";
import type { DomBuildResult, DomReadResult, DomResourceRecord } from "./public-types.js";
...
type CliFlags = Map<string, string | true>;

function argumentsOf(argv: readonly string[]): { positional: string[]; flags: CliFlags } {
...
async function encodeManifest(input: string, flags: CliFlags): Promise<DomBuildResult> {
...
function extensionFor(record: DomResourceRecord): string {
...
async function decodeFile(input: string, flags: CliFlags): Promise<DomReadResult> {
  ...
  try {
    ...
    for (const record of result.document.resources.resources) {
      const bytes = result.resourceBytes.get(record.id);
      invariant(bytes, "MISSING_EXTERNAL_RESOURCE", `Resource ${record.id} bytes are missing after validation.`);
      await writeFile(join(resourceDirectory, `${record.id}${extensionFor(record)}`), bytes, { flag: "wx" });
    }
  } catch (error) {
...
export async function main(argv: readonly string[] = process.argv.slice(2)): Promise<number> {
  try {
    ...
    const code = unsafeCode && /^[A-Z][A-Z0-9_]*$/u.test(unsafeCode) ? unsafeCode : "INTERNAL_ERROR";
