...
interface TrianglePublicationOptions {
...
function convex(points: readonly number[]): boolean {
  let sign = 0;
...
function intersect(a0x: number, a0y: number, a1x: number, a1y: number, b0x: number, b0y: number, b1x: number, b1y: number): [number, number] | null {
  const rx = a1x - a0x;
  const ry = a1y - a0y;
  const sx = b1x - b0x;
  const sy = b1y - b0y;
...
function radialExpansion(points: readonly number[], amount: number): number[] {
  let centerX = 0;
  let centerY = 0;
  const count = points.length / 2;
  for (let index = 0; index < points.length; index += 2) {
...
function safeBleed(points: readonly number[], edgeIndex: number, requested: number): number {
...
function offsetStableTriangle(left: number, right: number, height: number, amount: number): readonly number[] {
...
function expandedTriangle(left: number, right: number, height: number, basis: Basis, mask: number, fallback: number, shared: number): readonly number[] {
...
function validBasis(value: unknown): value is Basis {
...
function computedTransform(
  ...
  plan: TrianglePlan,
  triangle: TrianglePublicationOptions,
  basisHint: unknown,
): string | null {
  ...
  let basis: Basis = validBasis(basisHint) ? basisHint : [0, 1, 2];
  ...
  const transform = affineTransform(values, plan.matrixDecimals);
...
export function preparedTriangleTransform(
  ...
  const points: Vec3[] = sourcePositions.map((position) => [position[2], position[0], position[1]]);
