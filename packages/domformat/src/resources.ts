...
function knownRecordKeys(value: Record<string, unknown>, allowed: ReadonlySet<string>, code: string, label: string): void {
  for (const key in value) {
    if (Object.hasOwn(value, key)) invariant(allowed.has(key), code, `${label} contains unsupported field ${key}.`);
  }
}

export function assertResourceId(value: unknown, label = "resource id"): string {
  invariant(typeof value === "string" && RESOURCE_ID.test(value), "INVALID_RESOURCE_ID", `${label} is invalid.`);
  return value;
}

export function assertSafeRelativePath(value: unknown, label = "resource path"): string {
...
export function validateResourceCatalog(catalog: unknown, limits: DomLimits): Set<string> {
  const catalogRecord = plainRecord(catalog, "INVALID_RESOURCES", "RCRD");
  ...
  for (const [index, value] of catalogRecord.resources.entries()) {
    ...
    if (record.kind === "image") {
      ...
      aggregateImagePixels += dimensions.width * dimensions.height;
      invariant(
    ...
    for (const existing of externalPaths) {
      invariant(
        ...
        `External resource path ${resourcePath} has a file/directory collision.`,
      );
    }
...
function pngDimensions(bytes: Uint8Array): ImageDimensions {
  ...
  invariant(dimensions && sawEnd && sawImageData && (colorType !== 3 || sawPalette), "IMAGE_MEDIA_MISMATCH", "PNG is missing required image, palette, or end chunks.");
  return dimensions;
}
...
function webpDimensions(bytes: Uint8Array): ImageDimensions {
  ...
  let offset = 12;
  let extended: ImageDimensions | undefined;
...
export function imageDimensions(bytes: Uint8Array, mediaType: unknown): ImageDimensions {
...
export function validateResourceBytes(record: DomResourceRecord, bytes: Uint8Array | undefined, limits: DomLimits): void {
  invariant(bytes instanceof Uint8Array, "MISSING_EXTERNAL_RESOURCE", `Resource ${record.id} bytes are absent.`);
  invariant(bytes.length === record.byteLength, "RESOURCE_SIZE_MISMATCH", `Resource ${record.id} byte length does not match RCRD.`);
  if (record.kind === "image") {
    invariant(record.dimensions, "INVALID_RESOURCE_DIMENSIONS", `Resource ${record.id} dimensions are absent.`);
...
export function materializeCss(
  ...
  options: { readonly scope?: string; readonly limits?: DomLimits } = {},
): string {
