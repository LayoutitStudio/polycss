...
function documentEnvelope(value: unknown): Record<string, unknown> {
...
function readDomInternal(value: unknown, options: DomReadOptions = {}) {
  ...
  const validated = validateDocumentInternal(envelope, options);
...
export function readDom(value: DomBytes, options: DomReadOptions = {}): DomReadResult {
  return readDomInternal(value, options).result as DomReadResult;
}

async function readDomFileInternal(path: string, options: DomReadFileOptions = {}): Promise<DomReadResult> {
  const limits = mergeLimits(options.limits);
  ...
  try {
    ...
    if (errorCode(error) === "ENOENT") fail("MISSING_EXTERNAL_RESOURCE", "The resource directory is missing.");
    throw error;
  }
  const externalResources = new Map<string, Uint8Array>();
  for (const record of initial.document.resources.resources) {
    try {
      ...
      if (errorCode(error) === "ENOENT") fail("MISSING_EXTERNAL_RESOURCE", `Resource ${record.id} is missing.`);
      throw error;
    }
  }
...
export async function readDomFile(path: string, options: DomReadFileOptions = {}): Promise<DomReadResult> {
  try {
    return await readDomFileInternal(path, options) as DomReadResult;
  } catch (error) {
    if (errorName(error) === "DomFormatError") throw error;
    const unsafeCode = errorCode(error);
    const code = unsafeCode === "ENOENT" ? "FILE_NOT_FOUND" : unsafeCode === "EACCES" ? "FILE_ACCESS_DENIED" : "FILE_IO";
