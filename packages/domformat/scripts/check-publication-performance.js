...
const regressionThresholds = Object.freeze({
  ...
  basis: "The 50 ms ceiling remains enforced for page-preparation tasks. General renderer scheduler tasks, cadence, and relative-speed observations have no hard gate without attribution or a repeated-run noise distribution.",
  cadenceGate: null,
...
function summarizeTrace(events, startPerf, endPerf, evidence) {
  ...
  const tasks = events.filter((event) => event.ph === "X" && event.name === PUBLICATION_MAIN_TASK_EVENT && event.pid === end.pid && event.tid === end.tid && event.ts < endTrace && event.ts + event.dur > startTrace);
  ...
  for (const event of events) {
    if (event.ph !== "X" || event.pid !== end.pid || event.tid !== end.tid || event.ts >= endTrace || event.ts + event.dur <= startTrace || event.name === PUBLICATION_MAIN_TASK_EVENT) continue;
