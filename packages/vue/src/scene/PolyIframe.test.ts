/**
 * <PolyIframe> Vue component tests. Mirrors the React + vanilla counterparts —
 * renders an <iframe> wrapper inside the scene's preserve-3d context with
 * the same world→CSS transform conventions as <PolyMesh>.
 */
...
import { PolyIframe, type PolyIframeProps } from "./PolyIframe";

let mountedApps: Array<{ container: HTMLElement; app: ReturnType<typeof createApp> }> = [];

function renderIframe(
  iframeProps: PolyIframeProps,
): HTMLElement {
    ...
    setup() {
      return () =>
        h(PolyCamera, {}, {
          default: () =>
            h(PolyScene, {}, {
              default: () => h(PolyIframe, iframeProps),
            }),
        });
    },
  });
  app.mount(container);
  mountedApps.push({ container, app });
  return container;
}
