...
describe("mountScene", () => {
  ...
  describe("update() triggering re-render", () => {
    ...
    it("update with changed showWalls triggers re-render", () => {
      ...
      // After re-render, walls may appear (depending on wall mask)
      // Just verifying that the update doesn't throw and completes
  ...
  describe("destroy()", () => {
    ...
    it("unsubscribes from controller on destroy", () => {
      ...
      // After destroy, updating the controller should not cause errors
      // (the subscription should be removed)
      expect(() => controller.updateCamera({ zoom: 2 })).not.toThrow();
    });
