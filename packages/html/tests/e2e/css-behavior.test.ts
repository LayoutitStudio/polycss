...
import { renderScene } from "../../src/headless";
import { injectBaseStyles } from "../../src/styles";
import { STYLE_ID, FLOOR_CLASS, LAYER_CLASS, WALL_CLASS, CEILING_CLASS, CUBE_CLASS, FACE_CLASS } from "@voxcss-core/types";
import { shadeWallFace, shadeCubeFace, shadeColor } from "@voxcss-core/color/lighting";
...
describe("Wall CSS transforms", () => {
  ...
  it("wall shading colors are applied as backgroundColor", () => {
    ...
    const backLeft = root.querySelector<HTMLElement>(`.${WALL_CLASS}--backLeft`);
    expect(backLeft).not.toBeNull();
    const expectedColor = shadeWallFace(wallColor, "bl");
    expect(backLeft!.style.backgroundColor).toBe(expectedColor);

    const backRight = root.querySelector<HTMLElement>(`.${WALL_CLASS}--backRight`);
    expect(backRight).not.toBeNull();
    const expectedBrColor = shadeWallFace(wallColor, "br");
    expect(backRight!.style.backgroundColor).toBe(expectedBrColor);
  });
});
...
describe("Face appearance inline styles", () => {
  ...
  it("colored cube has correct shaded background-color on each visible face", () => {
    ...
    expect(topFace!.style.backgroundColor).toBe(shadeCubeFace(color, "t"));

    const frFace = root.querySelector<HTMLElement>(`.${FACE_CLASS}--fr`);
    expect(frFace).not.toBeNull();
    expect(frFace!.style.backgroundColor).toBe(shadeCubeFace(color, "fr"));

    const flFace = root.querySelector<HTMLElement>(`.${FACE_CLASS}--fl`);
    expect(flFace).not.toBeNull();
    expect(flFace!.style.backgroundColor).toBe(shadeCubeFace(color, "fl"));
  });
  ...
  it("top face has delta=0, fr has delta=-15, fl has delta=-25", () => {
    ...
    // t: rgb(128,128,128), fr: rgb(113,113,113), fl: rgb(103,103,103)
    const topFace = root.querySelector<HTMLElement>(`.${FACE_CLASS}--t`);
    expect(topFace!.style.backgroundColor).toBe(shadeColor(color, 0));

    const frFace = root.querySelector<HTMLElement>(`.${FACE_CLASS}--fr`);
    expect(frFace!.style.backgroundColor).toBe(shadeColor(color, -15));

    const flFace = root.querySelector<HTMLElement>(`.${FACE_CLASS}--fl`);
    expect(flFace!.style.backgroundColor).toBe(shadeColor(color, -25));
  });
  ...
  it("default color #cccccc is used when no color is specified", () => {
    ...
    const topFace = root.querySelector<HTMLElement>(`.${FACE_CLASS}--t`);
    expect(topFace).not.toBeNull();
    expect(topFace!.style.backgroundColor).toBe(shadeCubeFace("#cccccc", "t"));
  });
});
