...
import type { HeadlessRenderHandle } from "../../src/headless";
import { createDomRenderer } from "../../src/renderer/domRenderer";
import type { SceneSnapshot } from "../../src/renderer/domRenderer";
...
import {
  ...
  shadeColor
} from "@voxcss-core/color/lighting";
import type { CubeFace, Voxel, WallsMask, GridContext } from "@voxcss-core/types";
import {
  CUBE_FACES,
  DEFAULT_WALL_COLOR,
  WALL_CLASS,
...
describe("Wall face shading colors", () => {
  ...
  it("each visible wall has background-color matching shadeWallFace", () => {
    ...
    for (const wall of Array.from(wallElements)) {
      ...
      if (wallFace) {
        const expected = shadeWallFace(wallColor, wallFace);
        expect(rgbEqual(bg, expected)).toBe(true);
      }
    }
  ...
  it("back-left wall is brighter than front-right wall (inverted lighting)", () => {
    // shadeWallFace inverts the cube face deltas:
    // bl uses cubeFace bl delta=-40, inverted to +40 -> brighter
    // fr uses cubeFace fr delta=-15, inverted to +15 -> less bright
    const wallColor = "#808080";
    const blShade = shadeWallFace(wallColor, "bl");
    const frShade = shadeWallFace(wallColor, "fr");
...
describe("Face appearance for all 6 cube faces", () => {
  ...
  it("each face has correct shaded background-color", () => {
    ...
    for (const faceEl of Array.from(faceElements)) {
      ...
      const expected = shadeCubeFace(color, face);
      const actual = el.style.backgroundColor;
      expect(rgbEqual(actual, expected)).toBe(true);
    }
...
describe("Grid overlay SVG generation", () => {
  ...
  it("floor element has --voxcss-floor-base color set", () => {
    ...
    // Floor base color = shadeColor(wallColor, 120)
    const expected = shadeColor(DEFAULT_WALL_COLOR, 120);
    expect(rgbEqual(floorBase, expected)).toBe(true);
  });
});
