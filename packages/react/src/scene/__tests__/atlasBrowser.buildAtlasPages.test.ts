/**
 * Smoke tests: buildAtlasPages canvas pipeline (React atlasBrowser copy)
 *
...
import { buildAtlasPages, packTextureAtlasPlansWithScale } from "../atlasBrowser";
...
function makeDoc(): Document {
  // Use the real happy-dom document so canvas.toBlob / toDataURL are available.
  return document;
}
...
describe("buildAtlasPages — smoke tests", () => {
  it("returns an empty array for empty pages input", async () => {
    const result = await buildAtlasPages([], "baked", makeDoc(), 1, neverCancelled);
  ...
  it("does not throw for a single solid-polygon page", async () => {
    ...
    await expect(
      buildAtlasPages(packed.pages, "baked", makeDoc(), atlasScale, neverCancelled),
    ).resolves.toBeDefined();
  });

  it("returns one TextureAtlasPage per packed page", async () => {
    const { packed, atlasScale } = buildPacked([SOLID_RECT, SOLID_TRIANGLE]);
    const pages = await buildAtlasPages(packed.pages, "baked", makeDoc(), atlasScale, neverCancelled);
  ...
  it("isCancelled early-exit: returns fewer pages when cancelled after first", async () => {
    ...
    if (packed.pages.length < 2) {
      // If both polygons pack onto one page, skip this test
      return;
    }
    let callCount = 0;
    ...
    const pages = await buildAtlasPages(packed.pages, "baked", makeDoc(), atlasScale, cancelAfterFirst);
  ...
  it("does not throw for dynamic lighting mode", async () => {
    const { packed, atlasScale } = buildPacked([SOLID_RECT]);
    await expect(
      buildAtlasPages(packed.pages, "dynamic", makeDoc(), atlasScale, neverCancelled),
    ).resolves.toBeDefined();
  });
  ...
  it("url field is present on each returned page (may be null in stub env)", async () => {
    ...
    for (const page of pages) {
      // url is either a string (data URL / blob URL) or null (stub canvas env)
