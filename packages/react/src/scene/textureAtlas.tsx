...
export function useTextureAtlas(
  ...
  const useProjectiveQuad = useFullRectSolid && projectiveQuadSupported();
  const solidTrianglePrimitive = resolveSolidTrianglePrimitive(strategies);
  const useStableTriangle = solidTrianglePrimitive !== null;
  const useCornerShapeSolid = !disableI && textureLighting !== "dynamic" && cornerShapeSupported();
  ...
  const atlasPlans = useMemo(
    () => plans.map((plan) => {
      ...
      // Exclude full-rect solids (<b> path) and clipped-solid polys (<u>/<i> path).
      const fullRect = isFullRectSolid(plan);
      const cornerShape = useCornerShapeSolid &&
        !fullRect &&
    ...
    [plans, textureLighting, useFullRectSolid, useProjectiveQuad, useStableTriangle, useCornerShapeSolid, useBorderShape, disableB],
  );
...
export function TextureBorderShapePoly({
  ...
  const iDisabled = disabledStrategies?.has("i") ?? false;
  const useIForFullRect = bDisabled && borderShapeSupported();
  const cornerShape = !iDisabled && !fullRect && cornerShapeSupported()
    ? cornerShapeGeometryForPlan(entry)
    : null;
