...
**CI enforces this and the package boundaries.** `pnpm check:boundaries` (`.github/scripts/check-boundaries.mjs`) fails on any import that violates the dependency table above, on deep imports into another package's `src`, and on node builtins outside the Node-only surfaces. It scans `.ts`/`.tsx`/`.mts`/`.cts` **and** `.mjs`/`.cjs`/`.js` sources (so the `packages/skills` CLI is covered) and applies the same allow-list to each package's manifest: a `dependencies`, `optionalDependencies`, or `peerDependencies` entry outside the package's allow-list fails even when nothing imports it yet. `devDependencies` are tooling and are not constrained. `pnpm check:mirrors` (`.github/scripts/check-mirrors.mjs`) keeps the copy discipline honest in three ways: react↔vue clones must stay byte-identical; the structurally-divergent vanilla↔framework mirrors (atlas pipeline, `voxelRenderer.ts`, `styles.ts`) are hash-pinned in `.github/mirror-lock.json`, so editing any of them fails CI until you re-pin with `pnpm check:mirrors --update`; and — the check that actually enforces mirroring — each mirror set is partitioned into renderer **lanes** (`packages/polycss`, `packages/react`, `packages/vue`) and diffed against a base ref (`--base`, else `origin/$GITHUB_BASE_REF`, else the merge-base with `origin/main`). If one lane changed and another did not, CI fails; re-pinning the lock cannot launder it, because the lane check never consults the lock. Intentional per-renderer divergence is declared in `.github/mirror-waivers.json` (set name → a non-empty reason, optionally scoped to specific files), which is echoed loudly in the CI log and which `--update` never writes, so every waiver shows up in the PR diff. The job running this check must check out with `fetch-depth: 0`; without history the lane check skips itself with a warning rather than passing silently. `pnpm typecheck` runs `tsc --noEmit` per package (after `pnpm build:packages`, which sibling type resolution needs).
